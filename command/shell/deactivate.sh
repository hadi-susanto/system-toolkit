...
__validate_options() {
    ...
    if (( ${#args_ref[@]} > 0 )); then
        log_error "The deactivate command does not accept positional arguments"
...
__deactivate_loader() {
    ...
    if ! shell_installed "$shell"; then
        ...
        log_warn "Forcing deactivation of the $shell shell loader even though it is not installed."
    fi
    ...
    if ! loader_active; then
        ...
        log_warn "Force deactivating shell loader: $shell"
    fi

    if ! deactivate_loader; then
        log_error "Failed to deactivate shell loader: $shell"

        return 1
    fi

    log_info "Deactivated shell loader: $shell"
}
...
main() {
    ...
    load_shell_interface \
        "$shell" "loader_active" "deactivate_loader" || return 1
    __parse_args options args "$@"
    __validate_options options args || return $?

    if (( options[FORCE] )); then
        log_warn "Force deactivation is enabled"
    fi

    __deactivate_loader "$shell" "${options[FORCE]}"
}
