#!/usr/bin/env bash
set -euo pipefail

source "$COMMON_LIB/common.sh"
source "$BIN_LIB/__executables.sh"
source "$BIN_LIB/__scope.sh"

##
# __parse_args <options_name> <args_name> [arguments...]
#
# Parses installation scope, force, all, and executable-name arguments.
#
# Parameters:
#   options_name    Name of the associative array that receives option state.
#   args_name       Name of the indexed array that receives executable names.
#   arguments       Command-line arguments to parse.
#
__parse_args() {
    local options_name="$1"
    local args_name="$2"
    shift 2

    local -n options_ref="$options_name"
    local -n args_ref="$args_name"
    local local_dir
    local global_dir

    local_dir="$(local_install_dir)" || return $?
    global_dir="$(global_install_dir)"

    options_ref=(
        [ALL]=0
        [CMD]="install"
        [FORCE]=0
        [GLOBAL]=0
        [INSTALL_DIR]="$local_dir"
        [INVALID_OPTION]=""
        [LOCAL]=0
        [SCOPE]="local"
    )
    args_ref=()

    while (( $# > 0 )); do
        case "$1" in
            -l | --local)
                options_ref[LOCAL]=1
                options_ref[SCOPE]="local"
                options_ref[INSTALL_DIR]="$local_dir"
                ;;
            -g | --global)
                options_ref[GLOBAL]=1
                options_ref[SCOPE]="global"
                options_ref[INSTALL_DIR]="$global_dir"
                ;;
            -f | --force)
                options_ref[FORCE]=1
                ;;
            -a | --all | all)
                options_ref[ALL]=1
                ;;
            --)
                shift
                args_ref+=("$@")
                break
                ;;
            -*)
                if [[ -z "${options_ref[INVALID_OPTION]}" ]]; then
                    options_ref[INVALID_OPTION]="$1"
                else
                    options_ref[INVALID_OPTION]+=", ${1}"
                fi
                ;;
            *)
                args_ref+=("$1")
                ;;
        esac

        shift
    done
}

__validate_options() {
    local options_name="$1"
    local args_name="$2"
    local -n options_ref="$options_name"
    local -n args_ref="$args_name"

    if [[ -n "${options_ref[INVALID_OPTION]}" ]]; then
        log_error "Unknown install option: ${options_ref[INVALID_OPTION]}"

        return 1
    fi

    if (( options_ref[LOCAL] && options_ref[GLOBAL] )); then
        log_error "Cannot combine local and global scope options"

        return 1
    fi

    if (( options_ref[ALL] && ${#args_ref[@]} > 0 )); then
        log_error "Cannot combine --all with executable names"

        return 1
    fi

    if (( ! options_ref[ALL] && ${#args_ref[@]} == 0 )); then
        log_error "At least one executable name or --all is required"

        return 1
    fi

    if (( options_ref[FORCE] )); then
        log_warn "Force installation is enabled"
    fi

    if [[ "${options_ref[SCOPE]}" == "global" ]] && (( EUID != 0 )); then
        log_error "Global installation requires elevated privileges"
        log_error "Run: sudo syskit-bin.sh install --global <name>"

        return 1
    fi

    if [[ -e "${options_ref[INSTALL_DIR]}" ]] &&
        [[ ! -d "${options_ref[INSTALL_DIR]}" ]]; then
        log_error "Installation path is not a directory: ${options_ref[INSTALL_DIR]}"

        return 1
    fi

    if [[ ! -d "${options_ref[INSTALL_DIR]}" ]] &&
        ! install -d -m 0755 -- "${options_ref[INSTALL_DIR]}"; then
        log_error "Failed to create installation directory: ${options_ref[INSTALL_DIR]}"

        return 1
    fi

    if ! install_dir_in_path "${options_ref[INSTALL_DIR]}"; then
        log_warn "${options_ref[INSTALL_DIR]} is not present in PATH"
        log_warn "Installed executables may not be available until PATH is configured"
    fi
}

__resolve_args_to_executables() {
    local args_name="$1"
    local executables_name="$2"
    local -n args_ref="$args_name"
    local -n executables_ref="$executables_name"
    local -A seen=()
    local name

    executables_ref=()

    for name in "${args_ref[@]}"; do
        resolve_bin_executable "$name" >/dev/null || return $?

        if (( seen["$name"] )); then
            continue
        fi

        seen["$name"]=1
        executables_ref+=("$name")
    done
}

__install_executable() {
    local name="$1"
    local options_name="$2"
    local -n options_ref="$options_name"
    local source="$SYSKIT_ROOT/bin/$name"
    local target="${options_ref[INSTALL_DIR]}/$name"

    if [[ -d "$target" ]] && [[ ! -L "$target" ]]; then
        log_error "Installation target is a directory: $target"

        return 1
    fi

    if [[ -e "$target" ]] || [[ -L "$target" ]]; then
        if (( ! options_ref[FORCE] )); then
            log_warn "Executable already exists; skipping: $target"

            return 0
        fi

        log_warn "Overwriting existing executable: $target"
    fi

    install -m 0755 -- "$source" "$target" || return $?

    log_info "Installed executable: $name"
}

main() {
    local -A options
    local -a args
    local -a executables
    local name
    local failed=0

    __parse_args options args "$@"
    if ! __validate_options options args; then
        bash "$BIN_LIB/help.sh" install >&2

        return 1
    fi

    if (( options[ALL] )); then
        list_bin_executables executables

        if (( ${#executables[@]} == 0 )); then
            log_error "No SysKit executables are available to install"

            return 1
        fi
    else
        __resolve_args_to_executables args executables || return $?
    fi

    for name in "${executables[@]}"; do
        if __install_executable "$name" options; then
            continue
        fi

        log_error "Failed to install executable: $name"
        failed=1
    done

    return "$failed"
}

main "$@"
